...
- `src/omakron/skills.py` lets a routine point at a skill folder. The prompt
  is that folder's SKILL.md, read at every launch, so the file in the person's
  shared repository is the program and the run record keeps the text sent.
